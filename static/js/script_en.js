...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- State Variables ---
    let surveyInterval; // To hold the interval ID so we can stop it
    const surveyModalEl = document.getElementById('surveyModal');
    if (!surveyModalEl) return; // Don't run if the modal isn't on the page

    const surveyModal = new bootstrap.Modal(surveyModalEl);
    
    const questionTextEl = document.getElementById('surveyQuestionText');
    const responseAreaEl = document.getElementById('surveyResponseArea');
    const otherInputWrapper = document.getElementById('surveyOtherInputWrapper');
    const otherTextInput = document.getElementById('surveyOtherText');
    const submitBtn = document.getElementById('submitSurveyResponseBtn');
    ...
    const fetchNextQuestion = async () => {
        try {
            const response = await fetch('/en-api/survey/next-question');
            if (!response.ok) {
                console.error('Failed to fetch survey question.');
                clearInterval(surveyInterval); // Stop trying if there's an error
                return;
            }
            const data = await response.json();

            if (data.status === 'complete' || !data.question_id) {
                console.log('Survey complete or no new questions.');
                clearInterval(surveyInterval); // Stop the polling
            } else {
                currentQuestion = data;
                displaySurveyModal(data);
                clearInterval(surveyInterval); // Stop polling once a question is shown
            }
        } catch (error) {
    ...
    const displaySurveyModal = (question) => {
        questionTextEl.textContent = question.question_text;
        responseAreaEl.innerHTML = ''; // Clear previous options
        // Reset class name for emoji/yes-no buttons
        responseAreaEl.className = 'd-flex justify-content-center align-items-center flex-wrap';
        otherInputWrapper.style.display = 'none'; // Hide 'other' input
        selectedResponse = null; // Reset selection
        ...
        switch (question.type) {
            case 'rating_emoji':
                ...
                emojis.forEach((emoji, index) => {
                    ...
                    button.className = 'btn btn-outline-secondary btn-lg me-2 survey-option';
                    button.innerHTML = `<span style="font-size: 2rem;">${emoji}</span>`;
                    button.dataset.value = index + 1;
                    responseAreaEl.appendChild(button);
                });
                break;
            ...
            case 'yes_no':
                ['Yes', 'No'].forEach(option => {
                    const button = document.createElement('button');
                    button.className = 'btn btn-outline-secondary btn-lg me-3 survey-option';
            ...
            case 'multiple_choice':
                responseAreaEl.className = 'd-block text-start w-100 px-4'; // Change layout for radios
                question.options.forEach(option => {
                    const wrapper = document.createElement('div');
                    wrapper.className = 'form-check mb-2';
                    wrapper.innerHTML = `
                        <input class="form-check-input survey-option-radio" type="radio" name="surveyChoice" id="survey-${option.replace(/\s+/g, '-')}" value="${option}">
                        <label class="form-check-label" for="survey-${option.replace(/\s+/g, '-')}">
                            ${option}
                        </label>
                    `;
                    responseAreaEl.appendChild(wrapper);
                });
                break;
        }
        surveyModal.show();
    ...
    const submitResponse = async () => {
        ...
        if (selectedResponse === 'Other' && otherTextInput.value.trim() !== '') {
            finalResponse = `Other: ${otherTextInput.value.trim()}`;
        }
        ...
        try {
            const response = await fetch('/en-api/survey/submit-response', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
            ...
            if (response.ok) {
                surveyModal.hide();
                // After submitting, restart the polling to check for the next question after the delay
                startSurveyPolling();
            } else {
    ...
    const startSurveyPolling = () => {
        ...
        surveyInterval = setInterval(fetchNextQuestion, 60000); // Poll every 60 seconds
    };
    
    // --- Event Listeners for Survey ---
    responseAreaEl.addEventListener('click', (e) => {
        const target = e.target.closest('.survey-option');
        if (target) {
            // Handle button-style selections (emoji, yes/no)
            document.querySelectorAll('.survey-option').forEach(btn => btn.classList.remove('active'));
            target.classList.add('active');
            selectedResponse = target.dataset.value;
        }
    });

    responseAreaEl.addEventListener('change', (e) => {
        const target = e.target.closest('.survey-option-radio');
        if (target) {
            // Handle radio-button selections
            selectedResponse = target.value;
            otherInputWrapper.style.display = (selectedResponse === 'Other') ? 'block' : 'none';
            if (selectedResponse === 'Other') {
...
// static/js/script_en.js

// ... (keep all your existing JavaScript code) ...

// =============================================================
// --- STUDENT ONBOARDING SURVEY LOGIC ---
// =============================================================
