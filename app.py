    ...
    try:
        with connection.cursor() as cursor:
            ...
            insert_data = (
                ...
                data.get('class'), data.get('board'), data.get('med_ins'),
        ...
        try:
            with connection.cursor() as cursor:
                ...
                if user and check_password_hash(user['pass_word'], password):
                    # ======================================================
                    # NEW LOGIC: Check for temporary password
                    # ======================================================
                    if password == '111111':
                        # User is using the temporary password.
                        # Force them to reset it by rendering the reset form.
                        return render_template('login.html', 
                                               show_reset_form=True, 
                                               user_id_to_reset=user['user_id'])
