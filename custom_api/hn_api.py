...
hn_api = Blueprint('hn_api', __name__, url_prefix='/hn-api')
...
"""
    ...
    if uploaded_gemini_file:
        logging.info(
            f"Adding file part with URI: {uploaded_gemini_file.uri}, MIME: {uploaded_gemini_file.mime_type}")
        try:
            ...
            logging.error(
                f"Error creating Part from URI {uploaded_gemini_file.uri}: {e}")
            # Optionally add a text part indicating the file error to the AI
            parts.append(types.Part.from_text(
                "(Error: Could not properly attach file content for analysis due to an internal issue.)"))
...
@hn_api.route('/solve-math', methods=['POST'])
def solve_math():
    ...
    try:
        ...
        if problem_file_storage and problem_file_storage.filename:
            logging.info(
                f"Received file: {problem_file_storage.filename}, MIME: {problem_file_storage.mimetype}")
            ...
            if not mime_type or (not mime_type.startswith('image/') and mime_type != 'application/pdf'):
                ...
                try:
                    suffix = os.path.splitext(problem_file_storage.filename)[
                        1] or mimetypes.guess_extension(mime_type) or ''
                    fd, temp_file_path = tempfile.mkstemp(suffix=suffix)
                    os.close(fd)
                    logging.info(
                        f"Saving uploaded file to temporary path: {temp_file_path}")
                    ...
                    logging.info(
                        f"Uploading temporary file {temp_file_path} to Gemini File API...")
                    # Use genai.upload_file which returns the File object
                    uploaded_gemini_file_obj = get_client().files.upload(
                        file=temp_file_path
                    )
                    # Assign the returned object, not append to a list
                    logging.info(
                        f"Successfully uploaded file via File API. URI: {uploaded_gemini_file_obj.uri}")

                except Exception as e:
                    logging.error(
                        f"Error during file processing or upload for {problem_file_storage.filename}: {e}", exc_info=True)
        ...
        if not problem_text and not uploaded_gemini_file_obj:
            logging.error(
                "No text problem provided and no file successfully processed.")
        ...
        prompt_parts = build_math_prompt_parts(
            problem_text=problem_text, uploaded_gemini_file=uploaded_gemini_file_obj)
        ...
        # --- Call the Gemini API ---
        logging.info(
            f"Sending request to Gemini model ({MODEL_NAME}) with {len(prompt_parts)} parts...")
        ...
        try:
            ...
            logging.warning(
                f"Accessing response.text failed. Response structure might be unexpected or blocked. Response: {response}", exc_info=True)
            ...
            logging.error(
                f"AttributeError accessing response data. Response: {response}", exc_info=True)
            ...
            logging.error(
                f"Unexpected error extracting text from Gemini response: {e}", exc_info=True)
        ...
        if not solution_text or not solution_text.strip():
            ...
            try:
                ...
                if finish_reason != "STOP":
                    logging.warning(
                        f"Generation finished unexpectedly: {finish_reason}")
                ...
                logging.warning(
                    "Could not determine finish reason from response candidates.")
        ...
        logging.error(
            f"An unexpected error occurred in /solve-math route: {e}", exc_info=True)
        # Basic error message classification
        # Provide type for debugging
...
@hn_api.route('/check', methods=['POST'])
def check():
    ...
    try:
        ...
        if solution_file_storage and solution_file_storage.filename:
            logging.info(
                f"Received file: {solution_file_storage.filename}, MIME: {solution_file_storage.mimetype}")
            ...
            if not mime_type or (not mime_type.startswith('image/') and mime_type != 'application/pdf'):
                ...
                try:
                    suffix = os.path.splitext(solution_file_storage.filename)[
                        1] or mimetypes.guess_extension(mime_type) or ''
                    fd, temp_file_path = tempfile.mkstemp(suffix=suffix)
                    os.close(fd)
                    logging.info(
                    ...
                    logging.error(
                        f"Error during file processing or upload for {solution_file_storage.filename}: {e}", exc_info=True)
                    uploaded_gemini_file_obj = None  # Ensure it's None if upload failed
                    # return jsonify({"error": "Failed to process the uploaded file."}), 500 # Optional: fail request
        ...
        # --- Construct Prompt Parts (using the revised helper) ---
        # Pass the single uploaded file object (or None)
        prompt_parts = build_check_prompt_parts(
            problem_text=problem_text, uploaded_gemini_file=uploaded_gemini_file_obj)
...
"""
    if original_file_uri:
        ...
        try:
            ...
            logging.info(
                f"Including file URI {original_file_uri} in clarification prompt text.")
        except Exception as e:
            logging.warning(
                f"Could not create Part from URI {original_file_uri} for clarification: {e}")
...
@hn_api.route('/clarify-step', methods=['POST'])
def clarify_step():
    ...
    try:
        ...
        # Get the URI stored by frontend
        original_file_uri = data.get('originalProblemFileURI')
        ...
        try:
            if not response.candidates:
                logging.error(
                    f"Clarification response blocked or empty. Prompt Feedback: {response.prompt_feedback}")
            ...
            logging.info(
                f"Received clarification text length: {len(clarification_text)}")

        except (ValueError, AttributeError, IndexError) as e:
            logging.error(
                f"Error extracting text from clarification response: {e}. Response: {response}", exc_info=True)
        ...
        if not clarification_text or not clarification_text.strip():
            ...
            if finish_reason != "STOP":
                logging.warning(
                    f"Clarification generation finished unexpectedly: {finish_reason}")
        ...
        logging.error(
            f"An unexpected error occurred in /clarify-step route: {e}", exc_info=True)
        return jsonify({"error": f"An internal server error occurred during clarification: {type(e).__name__}"}), 500


@hn_api.route('/practice', methods=['POST'])
def practice():
    ...
    try:
        ...
        # Get the URI stored by frontend
        no_probs = data.get('numberOfProblems')
        ...
        try:
            ...
            logging.info(
                f"Received practice text length: {len(practice_problems)}")
...
# --- NEW ENDPOINT for Step Clarification ---
@hn_api.route('/refresher', methods=['POST'])
def refresher():
    ...
    raw_ai_content = concept_desc.strip()

    if not raw_ai_content:
        logging.error("Refresher AI returned empty content.")
        # Return error if empty
        return jsonify({"error": "AI returned empty refresher content. Cannot display."}), 500
    ...
    if match:
        ...
        logging.warning(
            "No ```json ``` block found. Attempting to parse raw content as JSON.")
    ...
    try:
        ...
        resp = json.loads(json_content_to_parse)
        # Use .get() with default to prevent KeyError
        concept_name = resp.get("concept_name", "Concept Details")
        concept_description = resp.get(
            "concept_description", "Description not available.")
        ...
        solved_questions = solved_questions_raw if isinstance(
            solved_questions_raw, list) else []
        ...
        logging.error(
            f"JSON Decode Error in /refresher after extraction attempt: {e}")
        ...
        return jsonify({"error": f"Failed to parse AI response as JSON: {e}. Raw content received: {raw_ai_content[:500]}..."}), 500
    except Exception as e:
        logging.error(
...
@hn_api.route('/submit-feedback', methods=['POST'])
def submit_feedback():
